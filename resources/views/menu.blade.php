...
@section('content')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 lg:gap-10">
                    @foreach ($category->items as $item)
                        <div class="gallery-demo"
                            onclick="showModal('{{ $item->name }}','{{ $item->price_per_unit }}','{{ $item->getFirstMediaUrl('item_image') }}','{{ $item->id }}')">
                            <a href="#gal1">
    ...
    <script>
        showModal = (name, price, image_url, item_id) => {
        ...
        <div class="popup">
            ...
            <h4 class="p-mask"> Amount- <span> <input type="number" placeholder="quantity" name="quantity"/></span></h4>
            </div>
